...
TEST_CASE("rebase policy require_special_seen", "[unit]") {
	...
	s.newest_special = h('t');
	CHECK(needs_rebase(s));
	s.special_ref = h('t'); // has seen the newest special
	CHECK(!needs_rebase(s));
	...
	// an empty reference (created before any special was in sync) is behind as well, even
	// when the record is based on a block beyond the special (tag bound, plan 4.3)
	s.special_ref = {};
	s.last_seen = chain_block_id{sequence_number{7}, h('c')};
	CHECK(needs_rebase(s));
	// no special record known yet: nothing to reference
	s.newest_special = {};
	CHECK(!needs_rebase(s));
