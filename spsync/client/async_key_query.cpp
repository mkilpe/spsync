...
#include <securepath/remote_object/network/asio_encrypted_client.hpp>
...
{
	observer_.connect<ro::events::on_connect>([&]{ on_connect(); });
	observer_.connect<ro::events::on_disconnect>([&](error err){ on_disconnect(err); });
}
...
void async_key_query::on_key(std::future<std::optional<crypto::public_key>> f) {
	...
	try {
		...
		LOG_WARN("exception when querying key [ex=%]", ex);
